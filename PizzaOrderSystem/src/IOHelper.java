...
public class IOHelper {
    ...
    public static int getInt(int low, String prompt, int high) {
        ...
        do {
        	//even though this method returns an int we can still print stuff to the user
            System.out.print(prompt);
            numericEntryOK = false;
            try {
            	//stores the input as an integer
                numFromUser = screenInput.nextInt();
                //doesn't get to this point unless above isn't an int
                dummy = screenInput.nextLine();
                //if the flag turns to true then the while loop continues, so we want it to stop after numFromUser
                //we have this flag so that the while loop can continue running even though it doesn't have an integer to compare to `low` and `high`
                numericEntryOK = true;
              //executes if numFromUser isn't an int
            } catch (InputMismatchException e) {
                dummy = screenInput.nextLine();
                System.out.println(dummy + " is not an integer!");
                //temporarily making numFromUser an int so while loop works
                numFromUser = low;
            }
          ...
          //!numericEntryOK relevant when user enters a non-int
          //<low || >high are relevant when user enters an int out of range
...
public class IOHelper {
	...
	public static int getInt(int low, String prompt, int high) {
		int userNum;
		do {
			// printing the prompt to give the scanner something to capture
			System.out.print(prompt)
		}
		int finalPrompt = Integer.parseInt(prompt);
		//String errorPrompt = finalPrompt + "is not between" + low + "and" + high;
		if (low <= finalPrompt && high >= finalPrompt) {
			return finalPrompt;
		}
		return finalPrompt;
	} // end main method
